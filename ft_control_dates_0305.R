ft_control_date = function(city_names,endday.est.1st,daten){
  if (city_names=="Korea"){
    ...
    (control_date = c(endday.est.1st+1,as.Date("2020-08-23"),
                      as.Date("2020-09-28"),as.Date("2020-10-12"),
                      as.Date("2020-11-08"),as.Date("2020-11-10"),as.Date("2021-01-27"),daten+1))
  }
  idds = which(control_date-(daten+1)>0)
  
  nn = length(idds)
  
  if (nn>0){
    control_date_final = control_date[-idds]
  } else {
